...
| Adapter | Command |
|---------|---------|
| Built-in Kysely | `ppx @better-auth/cli@latest migrate` (applies directly) |
| Prisma | `ppx @better-auth/cli@latest generate --output prisma/schema.prisma` then `ppx prisma migrate dev` |
| Drizzle | `ppx @better-auth/cli@latest generate --output src/db/auth-schema.ts` then `ppx drizzle-kit push` |
