...
export default abstract class BaseSessionStore implements SessionStore {
  protected publisher = new Publisher<StoreEvents>();
  ...
  subscribe<E extends keyof StoreEvents>(event: E, cb: Subscriber<StoreEvents, E>) {
    this.publisher.subscribe(event, cb);
  }

  unsubscribe<E extends keyof StoreEvents>(event: E, cb: Subscriber<StoreEvents, E>) {
